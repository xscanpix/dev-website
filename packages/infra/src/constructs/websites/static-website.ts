 ...
 *
 *  Modified by: Svante Nilsson [2024]
 */
...
export class StaticWebsite extends Construct {
  ...
  private suppressCDKNagViolations = (props: StaticWebsiteProps) => {
    ...
    !props.distributionProps?.certificate &&
      ["AwsSolutions-CFR4"].forEach((RuleId) => {
    ...
    ["AwsSolutions-L1"].forEach((RuleId) => {
    ...
    ["AwsSolutions-IAM5"].forEach((RuleId) => {
      NagSuppressions.addStackSuppressions(
        stack,
        [
          {
    ...
    ["AwsSolutions-IAM4"].forEach((RuleId) => {
      NagSuppressions.addStackSuppressions(
        stack,
        [
          {
    ...
    ["AwsSolutions-S1"].forEach((RuleId) => {
      NagSuppressions.addStackSuppressions(
        stack,
        [
          {
